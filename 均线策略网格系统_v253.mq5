...
void DrawPanel(EAStats &s)
{
   ...
   ELbl(g_prefix+"e2_l2","顺势间距", LX+120,  ey+LH+2, F(10), cMute);
   EEdt(g_prefix+"e3_trend", IntegerToString(g_gridWithTrend), LX+170,ey+LH,60,EH);
   ELbl(g_prefix+"e2_u2","点",     LX+234,ey+LH+2, F(9), cMute);

   // 第3行: 对冲比例 [____]    最小对冲 [____]
   ELbl(g_prefix+"e3_l1","对冲比例", LX,      ey+LH*2+2, F(10), cMute);
   EEdt(g_prefix+"e4_hedge", DoubleToString(g_hedgeRatio,2), LX+48,ey+LH*2,60,EH);
   ELbl(g_prefix+"e3_l2","最小对冲", LX+120,  ey+LH*2+2, F(10), cMute);
   EEdt(g_prefix+"e5_minP", DoubleToString(g_hedgeMinProfit,1), LX+170,ey+LH*2,60,EH);

   // 第4行: 最大层数 [____]    层均递增 [____]
   ELbl(g_prefix+"e4_l1","最大层数", LX,      ey+LH*3+2, F(10), cMute);
   EEdt(g_prefix+"e6_maxL", IntegerToString(g_maxLayers), LX+48,ey+LH*3,60,EH);
   ELbl(g_prefix+"e4_l2","层均递增", LX+120,  ey+LH*3+2, F(10), cMute);
   EEdt(g_prefix+"e7_inc",  DoubleToString(g_lotIncrement,2), LX+170,ey+LH*3,60,EH);
