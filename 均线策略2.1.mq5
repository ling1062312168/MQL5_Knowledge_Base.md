...
//| 一次 tick 内尽可能多平, 连 0.01 手都平不掉则放弃                      |
//| 内置参数: 从上向下排序, 按盈利预算批量平仓                              |
//| 外置参数: 最低保留对冲盈利 (g_unlockMinHedgeBuy/Sell)               |
//+------------------------------------------------------------------+
void ProgressiveCloseLossOrders()
{
   if(!g_progressiveClose)
   {
      g_progressiveClose = true;
      ReadEdits();  // 读取最新面板参数(最低保留额等)
   }
   ...
   double minLot = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
   double budget = currentHedge - minHedge;  // 可用于平仓的盈利预算

   Print("[渐进平仓] 锁仓方向=", (g_lockDir==1?"多":(g_lockDir==-1?"空":"双向")),
         " 对冲盈利=$", DoubleToString(currentHedge,2),
         " 最低保留=$", DoubleToString(minHedge,2),
         " 可用预算=$", DoubleToString(budget,2));
   ...
   for(int i=0; i<total; i++)
   {
      ...
      int mg = (int)PositionGetInteger(POSITION_MAGIC);
      if(mg==g_lockMagic) continue;  // 跳过锁仓单
      count++;
   }
   if(count==0)
   {
      Print("[渐进平仓] 无亏损单 → 平掉对冲单, 解锁");
      CloseLockHedge();
      ResetLockState();
      return;
   }
   ...
   // 排序: 按亏损最大的优先平
   // 多单亏损(价格跌): 开仓价高的亏损大 → 从上向下 (高→低)
   // 空单亏损(价格涨): 开仓价低的亏损大 → 从下向上 (低→高)
   bool highFirst = (g_lockDir == 1);  // 多单: 高价优先
   for(int i=0; i<count-1; i++)
      for(int j=i+1; j<count; j++)
      {
   ...
   // 批量平仓: 根据预算计算每张单能平多少手, 一次 tick 内尽可能多平
   int closedCnt = 0;
   ...
   for(int k=0; k<count; k++)
   {
      ...
      double orderLots   = entries[k].lots;
      double orderProfit = entries[k].profit;  // 负数表示亏损
      if(orderProfit >= 0) continue;  // 跳过盈利单或已回本的单

      double lossPerLot = (-orderProfit) / orderLots;  // 每手亏损金额 ($)
      ...
      if(closeLot < minLot)
      {
         ...
         if(closedCnt == 0)
         {
            Print("[渐进平仓] 预算不足最小单位,放弃平仓,保留对冲盈利$", DoubleToString(currentHedge,2));
         }
         break;
      }

      double released = closeLot * lossPerLot;
      if(ClosePosition(ticket, closeLot, 30))
      {
         closedCnt++;
         ...
         Print("[渐进平仓] 平单 #", closedCnt,
               ...
               " 释放亏损:$", DoubleToString(released,2),
   ...
   if(closedCnt > 0)
   {
      Print("[渐进平仓] 本次共平 ", closedCnt, " 笔 ", DoubleToString(totalClosedLots,2),
            "手, 释放亏损=$", DoubleToString(totalReleased,2));
   }
   else
   {
      Print("[渐进平仓] 无可平仓单 → 保留对冲盈利,等待下一tick重新计算");
   }
}
...
void CheckLock()
{
   ...
   if(g_locked)
   {
      // 方式1: 对冲单被全部平仓 (人工/面板平仓) → 直接解锁
      if(!HasLockHedge())
      {
         Print("════════════════════════════════");
         Print("[自动解锁] 锁仓对冲单已全部平仓 → 恢复交易");
         ResetLockState();
         return;
      }
      ...
      if(g_progressiveClose)
      {
         ...
         double minHedge = (g_lockDir==1) ? g_unlockMinHedgeBuy :
                           (g_lockDir==-1) ? g_unlockMinHedgeSell :
                           g_unlockMinHedgeBuy;   // 双向按多单处理

         ENUM_POSITION_TYPE lossType = (g_lockDir==1 || g_lockDir==0) ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;

         // 无亏损单 → 平掉对冲单, 解锁
         if(!HasLossOrders(lossType))
         {
            Print("[渐进平仓] 亏损单已全部平仓 → 平掉剩余对冲单并解锁");
            CloseLockHedge();
            ResetLockState();
            return;
         }

         // 对冲盈利低于最低保留额 → 停止渐进, 平掉对冲单, 解锁
         if(currentHedge < minHedge)
         {
            Print("[渐进平仓] 对冲盈利=$", DoubleToString(currentHedge,2),
                  " < 最低保留=$", DoubleToString(minHedge,2),
                  " → 平掉剩余对冲单并解锁");
            CloseLockHedge();
            ResetLockState();
         ...
         // 继续渐进平仓 (批量: 按预算一次平多笔)
         ProgressiveCloseLossOrders();
         return;
      }

      // 方式2: 对冲单盈利达解锁比例 → 进入渐进平仓模式
      double ratio = 0.30;
      if(g_lockDir == 1)       ratio = g_unlockRatioBuy;
      else if(g_lockDir == -1) ratio = g_unlockRatioSell;
      else                     ratio = (g_unlockRatioBuy + g_unlockRatioSell) / 2.0;
      ...
      if(g_lockOrigThresh > 0 && hedgeProfit >= targetProfit)
      {
         ...
         Print("[渐进平仓] 对冲盈利:$",DoubleToString(hedgeProfit,2),
               ...
               " (锁仓阈值$",DoubleToString(g_lockOrigThresh,2),"×",
               DoubleToString(ratio*100,0),"%)");
         ProgressiveCloseLossOrders();
      }
      return;
   }
   ...
   if(needLock)
   {
      ...
      g_locked          = true;
      // 判断触发方向
      if(buyDD >= g_lockOrigThresh && sellDD >= g_lockOrigThresh) g_lockDir = 0;   // 双向
      else if(buyDD >= g_lockOrigThresh)                          g_lockDir = 1;   // 多单触发
      else if(sellDD >= g_lockOrigThresh)                         g_lockDir = -1;  // 空单触发
      g_progressiveClose = false;  // 重置渐进平仓状态
      Print("════════════════════════════════");
      ...
      Print("[锁仓] 解锁方式: ①对冲单盈利≥$",DoubleToString(g_lockOrigThresh*unlockR,2),
            " (",DoubleToString(unlockR*100,0),"%)渐进平仓解锁 ②手动平掉对冲单");
      Print("════════════════════════════════");
      RefreshPanel(true);
      ObjectSetString(0, g_prefix+"e3_lock", OBJPROP_TEXT, "禁用");
   }
