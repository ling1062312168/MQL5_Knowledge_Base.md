...
#define CH_PARAMS       273       // 参数设置卡(6行输入/显示+1行按钮, 与右栏对齐)
// 左栏总高
...
void CpAsyncTick()
{
   if(g_locked) return;  // 锁仓后完全冻结, 禁止平仓
   CTrade cpTrade;
   ...
   for(int idx=0; idx<2; idx++)
   {
      ...
      for(int i=PositionsTotal()-1;i>=0 && tcnt<100;i--)
      {
         ...
         if(PositionGetSymbol(i)!=_Symbol) continue;
         if((ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE)!=pt) continue;
         // 仅平本EA的持仓: 自动单(g_currentMagic) + 手动单(g_manualMagic)
...
void DrawPanel(EAStats &s)
{
   ...
   ELbl(g_prefix+"r10_val", g_locked?"已锁仓":"正常", LX+CD_PD+LW*4/10,ry+LH*9+1, F(10), g_locked?cBad:cOk);
    ...
    // 第1行: [____]手数    [____]止盈
    EEdt(g_prefix+"e1_lot",  DoubleToString(g_lot,2),         LX+CD_PD+2,  ey,    EDT_W,EH);
    ELbl(g_prefix+"e1_l1","手数",   LX+CD_PD+2+EDT_W+2,    ey+2,     F(10), cMute);
    EEdt(g_prefix+"e1_tp",   IntegerToString(g_tp),           LX+CD_PD+LW/2+2, ey, EDT_W,EH);
    ELbl(g_prefix+"e1_l2","止盈",   LX+CD_PD+LW/2+2+EDT_W+2, ey+2,     F(10), cMute);

    // 第2行: [____]止损    [____]浮亏阈值
    EEdt(g_prefix+"e1_sl",   IntegerToString(g_sl),           LX+CD_PD+2,  ey+LH,EDT_W,EH);
    ELbl(g_prefix+"e2_l1","止损",   LX+CD_PD+2+EDT_W+2,    ey+LH+2,  F(10), cMute);
    EEdt(g_prefix+"e3_lock", DoubleToString(g_lockDrawdownUSD,0),LX+CD_PD+LW/2+2,ey+LH,EDT_W,EH);
    ELbl(g_prefix+"e3_l1","浮亏阈值",LX+CD_PD+LW/2+2+EDT_W+2,ey+LH+2,  F(10), cMute);

    // 第3行: [____]多单最大层    [____]空单最大层
    EEdt(g_prefix+"e6_maxLB",IntegerToString(g_maxLayersBuy), LX+CD_PD+2,ey+LH*2,EDT_W,EH);
    ELbl(g_prefix+"e6_l1","多单最大层",LX+CD_PD+2+EDT_W+2,   ey+LH*2+2,F(10), cMute);
    EEdt(g_prefix+"e6_maxLS",IntegerToString(g_maxLayersSell),LX+CD_PD+LW/2+2,ey+LH*2,EDT_W,EH);
    ELbl(g_prefix+"e6_l2","空单最大层",LX+CD_PD+LW/2+2+EDT_W+2,ey+LH*2+2,F(10), cMute);

    // 第4行: [____]多单递增    [____]空单递增
    EEdt(g_prefix+"e7_incB", DoubleToString(g_lotIncrementBuy,2), LX+CD_PD+2,ey+LH*3,EDT_W,EH);
    ELbl(g_prefix+"e7_l1","多单递增",LX+CD_PD+2+EDT_W+2,     ey+LH*3+2,F(10), cMute);
    EEdt(g_prefix+"e7_incS", DoubleToString(g_lotIncrementSell,2),LX+CD_PD+LW/2+2,ey+LH*3,EDT_W,EH);
    ELbl(g_prefix+"e7_l2","空单递增",LX+CD_PD+LW/2+2+EDT_W+2,ey+LH*3+2,F(10), cMute);

    // 第5行: [____]多单总手限    [____]空单总手限
    EEdt(g_prefix+"e8_maxLB",DoubleToString(g_maxTotalLotsBuy,2), LX+CD_PD+2,ey+LH*4,EDT_W,EH);
    ELbl(g_prefix+"e8_l1","多单总手限",LX+CD_PD+2+EDT_W+2,   ey+LH*4+2,F(10), cMute);
    EEdt(g_prefix+"e8_maxLS",DoubleToString(g_maxTotalLotsSell,2),LX+CD_PD+LW/2+2,ey+LH*4,EDT_W,EH);
    ELbl(g_prefix+"e8_l2","空单总手限",LX+CD_PD+LW/2+2+EDT_W+2,ey+LH*4+2,F(10), cMute);

    // 第6行: [____]多单指数倍间距    [____]空单指数倍间距
    EEdt(g_prefix+"e10_expB", DoubleToString(g_gridExpFactorBuy,2),LX+CD_PD+2,ey+LH*5,EDT_W,EH);
    ELbl(g_prefix+"e10_l1","多单指数倍间距",LX+CD_PD+2+EDT_W+2,  ey+LH*5+2,F(10), cMute);
    EEdt(g_prefix+"e10_expS", DoubleToString(g_gridExpFactorSell,2),LX+CD_PD+LW/2+2,ey+LH*5,EDT_W,EH);
    ELbl(g_prefix+"e10_l2","空单指数倍间距",LX+CD_PD+LW/2+2+EDT_W+2,ey+LH*5+2,F(10), cMute);
   ...
   {
      double clipL_fx = MathMin(fixLVal, s.buy_lot);
   ...
   {
      double clipL_or = MathMin(ordLVal, s.buy_lot);
   ...
   {
      double clipS_fx = MathMin(fixSVal, s.sell_lot);
   ...
   {
      double clipS_or = MathMin(ordSVal, s.sell_lot);
...
void SyncStateFromPositions()
{
   ...
   // ── 手数基数恢复默认 (外部平仓后无法精确反推, 用默认值最安全) ──
   // 注: 这会覆盖面板上修改过的 g_lotBase, 若需要在面板保留请注释下行
   if(totalPos == 0) g_lotBase = InpLotSize;
}
//+------------------------------------------------------------------+
//| 指标                                                              |
//+------------------------------------------------------------------+
...
void DoBuy()
{
   ...
   double nextLot = g_lotBase + g_gridLayer * g_lotIncrementBuy;
...
void DoSell()
{
   ...
   double nextLot = g_lotBase + g_gridLayer * g_lotIncrementSell;
   if(nextLot < InpLotSize) nextLot = InpLotSize;
