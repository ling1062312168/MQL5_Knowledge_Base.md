...
// 右栏卡片高度
// 多单平仓卡
#define CH_ACT_MULTI    (16+18+8+LH+PG+BH+PG+LH+PG+BH+PG+cp_RowH+PG+cp_RowH+PG+cp_RowH+PG+cp_RowH+PG*2+16)
// 空单平仓卡
#define CH_ACT_SHORT    (16+18+8+LH+PG+BH+PG+cp_RowH+PG+cp_RowH+PG+cp_RowH+PG+cp_RowH+16)
// 运行信息卡: 16头+18标题+8间隔+4信息行(4×22=88)+16底 = 154
#define CH_INFO         (16+18+8 + 4*LH + 16)
// 右栏总高(多单卡+间距+空单卡+间距+信息卡)
#define RIGHT_COL_H     (CH_ACT_MULTI + SG + CH_ACT_SHORT + SG + CH_INFO)
   ...
   for(int idx=0; idx<2; idx++)
   {
      ...
      if(mode == 5)
      {
         ...
         for(int k=0;k<tcnt && closedLots<val;k++)
         {
            ...
            {
               // 部分平仓
               if(cpTrade.PositionClosePartial(tickets[k], closeLots, 30))
               {
      ...
      if(mode == 6)
      {
         ...
         for(int k=0;k<tcnt && closedLots<val;k++)
         {
            ...
            {
               if(cpTrade.PositionClosePartial(sorted[k], closeLots, 30))
               {
                  closedLots += closeLots;
                  g_cpAsyncTkt[idx] = TimeCurrent();
...
void DrawPanel(EAStats &s)
{
   ...
   else
   {
      ...
      for(int i=PositionsTotal()-1;i>=0;i--)
      {
         ...
         {
            l=-(PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_COMMISSION)+PositionGetDouble(POSITION_SWAP));
            if(l<=0) continue;
            cLoss+=l; cCnt++;
         }
      }
   ...
   EEdt(g_prefix+"e1_lot", DoubleToString(g_lot,2), LX+48,ey,cp_EW1,EH);
   ELbl(g_prefix+"e1_l2","止 盈", LX+48+cp_EW1+PG,  ey+2, F(10), cMute);
   EEdt(g_prefix+"e1_tp",  IntegerToString(g_tp), LX+48+cp_EW1+PG+48,ey,cp_EW2,EH);
   ELbl(g_prefix+"e1_u2","点",     LX+48+cp_EW1+PG+48+cp_EW2+4,ey+2, F(9), cMute);

   // 第2行: 止损 [____] 点
   ELbl(g_prefix+"e2_l2","止 损", LX+48+cp_EW1+PG,  ey+LH+2, F(10), cMute);
   EEdt(g_prefix+"e1_sl",  IntegerToString(g_sl), LX+48+cp_EW1+PG+48,ey+LH,cp_EW2,EH);
   ELbl(g_prefix+"e2_u2","点",     LX+48+cp_EW1+PG+48+cp_EW2+4,ey+LH+2, F(9), cMute);

   // 第3行: 顺势间距 [____] 点 (新增)
   ELbl(g_prefix+"e3_l1","顺势间距", LX,      ey+LH*2+2, F(10), cMute);
   EEdt(g_prefix+"e3_trend", IntegerToString(g_gridWithTrend), LX+48,ey+LH*2,cp_EW2,EH);
   ELbl(g_prefix+"e3_u1","点",    LX+48+cp_EW2+4,ey+LH*2+2, F(9), cMute);

   // 第4行: 对冲比例 [____] (新增)
   ELbl(g_prefix+"e4_l1","对冲比例", LX,      ey+LH*3+2, F(10), cMute);
   EEdt(g_prefix+"e4_hedge", DoubleToString(g_hedgeRatio,2), LX+48,ey+LH*3,cp_EW2,EH);

   // 第5行: 最小对冲盈利 [$____] (新增)
   ELbl(g_prefix+"e5_l1","最小对冲", LX,      ey+LH*4+2, F(10), cMute);
   EEdt(g_prefix+"e5_minP", DoubleToString(g_hedgeMinProfit,1), LX+48,ey+LH*4,cp_EW2,EH);
...
//      右栏: 多单平仓 + 空单平仓(双卡)
   ...
   // ===== 卡片3: 多单平仓管理 =====
   ERect(g_prefix+"c3",rx-PD,cy,RW_+PD*2,CH_ACT_MULTI,BG_CARD,BD_PANEL);
   ELbl(g_prefix+"c3_title","多单平仓管理",rx,cy+PD,F(12),InpColorBuy);
   by = cy + PD + 22;
   ...
   ELbl(g_prefix+"cp_totalPnl","浮动盈亏 = "+pnlSign+DoubleToString(totalPnL,2),
        rx+PD, by, F(11), pnlClr);
   by += LH + PG;

   // --- 一键平仓 ---
   cbw = RW_ - PD*2;
   EBtn(g_prefix+"btn_closeAll","一键平仓", rx+PD, by, cbw, BH, C'180,50,50', cWhite);
   by += BH + PG;

   // --- 多单平仓管理 ---
   ELbl(g_prefix+"cp_secL","多仓单管理", rx+PD, by, F(11), InpColorBuy);
   by += LH + PG;

   // 全平 | 盈 | 买
   bw3 = (RW_ - PD*2 - PG*2) / 3;
   EBtn(g_prefix+"cp_allL","全平多单", rx+PD, by, bw3, BH, C'56,132,216', cWhite);
   EBtn(g_prefix+"cp_profL","平盈利多单", rx+PD+bw3+PG, by, bw3, BH, C'78,190,140', cWhite);
   EBtn(g_prefix+"cp_lossL","平亏损多单", rx+PD+bw3*2+PG*2, by, bw3, BH, C'230,100,97', cWhite);
   by += BH + PG;

   // 当前多单统计
   buyStat = "当前多单手数:"+DoubleToString(s.buy_lot,2)+"  数量:"
                     +IntegerToString(s.buy_cnt)+"  盈亏:$"+DoubleToString(s.buy_pnl,2);
   buyStatClr = (s.buy_pnl >= 0) ? cOk : cBad;
   ELbl(g_prefix+"cp_buyStat", buyStat, rx+PD, by, F(9), buyStatClr);
   by += cp_RowH + PG;

   // 百分比平仓: [标签] [输入] [抽取] [=结果]
   EBtn(g_prefix+"cp_perL_btn", "百分比平仓", rx+PD, by, cp_LblW, cp_RowH, C'70,72,85', cWhite);
...
perLClr = cMute;
   ...
   // 固定手数平仓
   EBtn(g_prefix+"cp_fixL_btn", "固定手数平仓", rx+PD, by, cp_LblW, cp_RowH, C'70,72,85', cWhite);
...
ordLClr = cMute;
   ...
   // ===== 卡片4: 空单平仓管理 =====
   cy = g_py + HDR_H + SG + CH_ACT_MULTI + SG;
   ERect(g_prefix+"c4",rx-PD,cy,RW_+PD*2,CH_ACT_SHORT,BG_CARD,BD_PANEL);
   ELbl(g_prefix+"c4_title","空单平仓管理",rx,cy+PD,F(12),InpColorSell);
   by = cy + PD + 22;
   ELbl(g_prefix+"cp_secS","空仓单管理", rx+PD, by, F(11), InpColorSell);
   by += LH + PG;

   // 全平 | 盈 | 买
   bw3S = (RW_ - PD*2 - PG*2) / 3;
   EBtn(g_prefix+"cp_allS","全平空单", rx+PD, by,bw3S, BH, C'180,50,50', cWhite);
   EBtn(g_prefix+"cp_profS","平盈利空单", rx+PD+bw3S+PG, by, bw3S, BH, C'78,190,140', cWhite);
   EBtn(g_prefix+"cp_lossS","平亏损空单", rx+PD+bw3S*2+PG*2, by, bw3S, BH, C'230,100,97', cWhite);
   by += BH + PG;

   // 当前空单统计
   sellStat = "当前空单手数:"+DoubleToString(s.sell_lot,2)+"  数量:"
                      +IntegerToString(s.sell_cnt)+"  盈亏:$"+DoubleToString(s.sell_pnl,2);
   sellStatClr = (s.sell_pnl >= 0) ? cOk : cBad;
   ELbl(g_prefix+"cp_sellStat", sellStat, rx+PD, by, F(9), sellStatClr);
   by += cp_RowH + PG;

   // 百分比平仓
   EBtn(g_prefix+"cp_perS_btn", "百分比平仓", rx+PD, by, cp_LblW, cp_RowH, C'70,72,85', cWhite);
...
perSClr = cMute;
   ...
   // 固定手数平仓
   EBtn(g_prefix+"cp_fixS_btn", "固定手数平仓", rx+PD, by, cp_LblW, cp_RowH, C'70,72,85', cWhite);
...
fixSClr = cMute;
   ...
   dirClrS = g_shortCloseDir ? C'70,72,85' : C'180,80,60';
...
ordSClr = cMute;
   ...
   // ===== 卡片c_info: 运行信息 =====
   cy = g_py + HDR_H + SG + CH_ACT_MULTI + SG + CH_ACT_SHORT + SG;
   ERect(g_prefix+"c_info",rx-PD,cy,RW_+PD*2,CH_INFO,BG_CARD,BD_PANEL);
   ELbl(g_prefix+"c_info_title","ℹ 运行信息",rx,cy+PD,F(12),cMute);
...
void CollectStats(EAStats &s)
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      p=m_pos.Profit()+m_pos.Commission()+m_pos.Swap();
      s.pnl+=p;
...
{
   double pt,bid,ask,v;
   double buyVol=0,buyWeighted=0,sellVol=0,sellWeighted=0;
...
double GetTrendProfit()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(!isTrend) continue;
      profit += m_pos.Profit() + m_pos.Commission() + m_pos.Swap();
   }
...
{
   double ask,availableProfit,bid,currentLoss,currentLots,dist,emaDeviation,entry,hedgeAmount,hedgeLots,l,loss,perLotLoss,pt,step,strengthFactor,trendProfit,remLoss;
   ...
   for(int i=PositionsTotal()-1; i>=0 && counterCnt<100; i--)
   {
      ...
      loss = -(m_pos.Profit()+m_pos.Commission()+m_pos.Swap());
   ...
   for(int i=0; i<counterCnt && availableProfit>=g_hedgeMinProfit; i++)
   {
      ...
      if(m_trade.PositionClosePartial(counter[i].ticket, hedgeLots, InpSlippage))
      {
         hedged = true;
   ...
   if(hedged)
   {
      remLoss=0; int remCnt=0, lastMagic=0;
      for(int i=PositionsTotal()-1;i>=0;i--)
      {
         if(!m_pos.SelectByIndex(i)) continue;
         if(m_pos.Symbol()!=_Symbol) continue;
         ...
         if((g_trend==1&&!isBuy)||(g_trend==-1&&isBuy))
         {
            l=-(m_pos.Profit()+m_pos.Commission()+m_pos.Swap());
            if(l<=0) continue;
...
void OnTick()
{
   ...
   if(g_lastTrend!=0 && g_trend!=0 && g_trend!=g_lastTrend)
   {
      ...
      int    oldCounterCnt=0,  newCounterCnt=0;
      ...
      for(int i=PositionsTotal()-1;i>=0;i--)
      {
         ...
         if(m_pos.Symbol()!=_Symbol) continue;
         bool isBuy=(m_pos.PositionType()==POSITION_TYPE_BUY);
         double p = m_pos.Profit()+m_pos.Commission()+m_pos.Swap();
