...
#property description "账户仓位多空仓位平衡风控 v5 - 仅监控和平仓, 不开仓"
#property description "v5: 修复触发逻辑/基准计算/标记持久化/方向自适应"
...
#define RW              LW
#define MARK_COMMENT    "BRC_MARK"
#define MONITOR_DISABLED -999999.0
#define PERSIST_PREFIX  "BRC_PERSIST_"
...
double         g_monitorOrigThresh = 0;
double         g_monitorDrawdownUSD = -999999.0;
double         g_unlockRatio    = 0.50;
double         g_minHedgeProfit = 5.0;
bool           g_progressiveClose = false;
double         g_balanceTolerance = 0.01;

ulong          g_markedTickets[];
string         g_markedSymbols[];
int            g_markDirections[];
double         g_markInitialLosses[];
int            g_monitorScroll   = 0;

datetime       g_lastMarkCheckTime = 0;
...
void SetPanelDragHighlight(bool on)
{
   ...
   color panelColor;
   int panelWidth;
   if(on) { panelColor = InpColorInfo; panelWidth = 2; }
   else { panelColor = C'45,58,74'; panelWidth = 1; }
...
void DrawToggle()
{
   ...
   ELbl(g_prefix+"title_collapsed", "账户仓位多空仓位平衡风控 v5 (已折叠)", x+PD+4, y+8, F(11), C'235,240,250');
...
void SaveMarkedTicketsToGV()
{
   ...
   for(int i=0; i<ArraySize(g_markedSymbols); i++)
   {
      ...
      string dirKey = PERSIST_PREFIX+"sym_"+IntegerToString(i)+"_dir";
      ...
      {
         ulong ticket = g_markedTickets[j];
         for(int k=(int)PositionsTotal()-1; k>=0; k--)
         {
            ulong t = PositionGetTicket(k);
            if(t==0) continue;
            if(!PositionSelectByTicket(t)) continue;
            if(PositionGetString(POSITION_SYMBOL) == g_markedSymbols[i] && t == ticket)
            {
               if(ticketStr != "") ticketStr += ",";
               ticketStr += IntegerToString((int)ticket);
               break;
            }
         }
      }
...
//+------------------------------------------------------------------+
//| 标记持久化: 从 GlobalVariable 恢复标记信息                         |
//+------------------------------------------------------------------+
void LoadMarkedTicketsFromGV()
{
   if(!GlobalVariableCheck(PERSIST_PREFIX+"sym_count")) return;
   ...
   for(int i=0; i<symCount; i++)
   {
      ...
      if(ticketStr != "")
      {
         ...
         while(commaPos >= 0)
         {
            ...
            if(ticketStr2 != "")
            {
               ulong ticket = (ulong)StringToInteger(ticketStr2);
...
//| 清理持久化数据                                                    |
//+------------------------------------------------------------------+
void ClearMarkedTicketsGV()
{
   int symCount = 0;
   if(GlobalVariableCheck(PERSIST_PREFIX+"sym_count"))
      symCount = (int)GlobalVariableGet(PERSIST_PREFIX+"sym_count");
   ...
   GlobalVariableDel(PERSIST_PREFIX+"progressive");
}
...
bool ClosePosition(ulong positionTicket, double volume, int slippage)
{
   ...
   // 平仓成功后取消标记
   UnmarkPositionForMonitor(positionTicket);
   return true;
}
...
//+------------------------------------------------------------------+
//| 获取品种的实际盈利方向 (返回盈利更大的方向)                         |
//+------------------------------------------------------------------+
int GetActualProfitDirection(string symbol)
{
   double buyProfit = GetDirectionProfit(symbol, 1);
   double sellProfit = GetDirectionProfit(symbol, -1);

   // 如果两个方向都是负的，返回亏损较少的方向
   if(buyProfit < 0 && sellProfit < 0)
   {
      return (buyProfit >= sellProfit) ? 1 : -1;
   }
...
//| v5优化: 自动检测盈利方向, 自适应预算计算                            |
//+------------------------------------------------------------------+
bool ProgressiveCloseLossOrders(string symbol, int markDirection)
{
   if(!g_progressiveClose) g_progressiveClose = true;

   // v5优化: 检测实际盈利方向
   double buyProfit = GetDirectionProfit(symbol, 1);
   double sellProfit = GetDirectionProfit(symbol, -1);

   int actualDir = markDirection;
   double directionProfit = GetDirectionProfit(symbol, markDirection);

   // 如果标记方向不是最盈利的方向，切换到实际盈利方向
   if(markDirection == 1 && sellProfit > buyProfit)
   {
      actualDir = -1;
      directionProfit = sellProfit;
      Print("[渐进平仓] 检测到空单盈利更多, 切换到空方向平仓");
   }
   else if(markDirection == -1 && buyProfit > sellProfit)
   {
      actualDir = 1;
      directionProfit = buyProfit;
      Print("[渐进平仓] 检测到多单盈利更多, 切换到多方向平仓");
   }

   double budget = directionProfit - g_minHedgeProfit;
   string dirName;
   if(actualDir==1) dirName = "多";
   else dirName = "空";
   ...
   // 收集所有已标记的非盈利方向的亏损单
   ENUM_POSITION_TYPE closeType = (actualDir == 1) ? POSITION_TYPE_SELL : POSITION_TYPE_BUY;
   ...
   // v5优化: 检查是否完成解锁
   double finalDirectionProfit = GetDirectionProfit(symbol, actualDir);
   Print("[渐进平仓] ",symbol," ",dirName,"方向盈利=$",DoubleToString(finalDirectionProfit,2),
         " 最低保留=$",DoubleToString(g_minHedgeProfit,2));
   ...
   for(int i=(int)PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(!PositionSelectByTicket(t)) continue;
      if(PositionGetString(POSITION_SYMBOL) != symbol) continue;
      if(IsMarkedOrder()) totalRemaining++;
   }
   if(totalRemaining == 0)
   {
      Print("[渐进平仓] ",symbol," 标记订单已清空 → 解锁");
      return true;
   }
...
void ResetMonitorState()
{
   ...
   ArrayResize(g_markedTickets, 0);

   if(InpMonitorDrawdownUSD == 0)
   {
      g_monitorDrawdownUSD = 0;
   ...
   ClearMarkedTicketsGV();
   RefreshPanel(true);
}

//+------------------------------------------------------------------+
//| 核心: 标记监控流程 (v5优化版)                                     |
//|   1. 已标记 → 检查任一方向盈利达50% → 渐进平仓 → 解锁            |
//|   2. 未标记 → 检测平衡品种 → 标记监控                            |
//|      - 阈值>0: 账户浮亏达阈值才触发                              |
//|      - 阈值=0: 自动监控所有平衡品种                              |
//|   ⚠️ 本函数仅做标记和平仓, 绝不开仓                               |
...
void CheckMonitor()
{
   ...
   if(g_monitoring)
   {
      ...
      for(int s=0; s<ArraySize(g_markedSymbols); s++)
      {
         ...
         // 优先检查实际盈利的方向
         if(buyProfit >= sellProfit && targetBuy > 0 && buyProfit >= targetBuy)
            triggerDir = 1;
         else if(sellProfit > buyProfit && targetSell > 0 && sellProfit >= targetSell)
            triggerDir = -1;
         ...
         // v5优化: 如果两个方向都没有盈利, 但有一个方向亏损减少到50%以下也触发
         if(triggerDir == 0)
         {
            // 检查亏损减少情况
            double buyLossReduced = 0, sellLossReduced = 0;
            if(curStats.buyPnl < 0 && initialLoss > 0)
               buyLossReduced = initialLoss + curStats.buyPnl; // curStats.buyPnl is negative
            if(curStats.sellPnl < 0 && initialLoss > 0)
               sellLossReduced = initialLoss + curStats.sellPnl;

            if(buyLossReduced > 0 && buyLossReduced >= initialLoss * g_unlockRatio)
            {
               triggerDir = 1;
               Print("[监控检查] ",sym," 多单亏损减少50% → 触发");
            }
            else if(sellLossReduced > 0 && sellLossReduced >= initialLoss * g_unlockRatio)
            {
               triggerDir = -1;
               Print("[监控检查] ",sym," 空单亏损减少50% → 触发");
            }
         }
   ...
   if(g_monitorDrawdownUSD == 0) shouldMonitor = true;
   ...
   for(int i=0; i<balancedCnt; i++)
   {
      ...
      // v5优化: 使用较小亏损作为50%目标的基准
      // 例: 多亏$100, 空亏$50 → initialLoss=$50 → 目标=$25
      // 当任一方向盈利达$25时触发渐进平仓
      int markDir = 1;
      double initialLoss = 0;

      if(buyLoss > 0 && sellLoss > 0)
      {
         // 两个方向都亏损, 使用较小的
         initialLoss = MathMin(buyLoss, sellLoss);
         markDir = (buyLoss <= sellLoss) ? 1 : -1;
      }
      else if(buyLoss > 0)
      {
         initialLoss = buyLoss;
      ...
      {
         initialLoss = sellLoss;
         markDir = -1;
      }
      else
      {
         // 没有亏损, 使用最小固定值
         initialLoss = MathMax(1.0, g_minHedgeProfit * 2);
         markDir = (buyLoss >= sellLoss) ? 1 : -1;
      }

      Print("[标记监控] ",sym," 基准亏损:$",DoubleToString(initialLoss,2),
            " 解锁目标:$",DoubleToString(initialLoss * g_unlockRatio,2));
   ...
   Print("[标记监控] 解锁方式: 任一方向盈利达"+DoubleToString(g_unlockRatio*100,0)+"% → 渐进平仓 → 解锁");
   Print("════════════════════════════════");
   RefreshPanel(true);
}
...
void DrawPanel()
{
   ...
   ELbl(g_prefix+"title", "账户仓位多空仓位平衡风控 v5", LX+4, g_py+8, F(14), C'235,240,250');
   ...
   if(g_monitoring && totalMarkedSymbols > 0)
   {
      ...
      for(int s=0; s<ArraySize(g_markedSymbols); s++)
      {
         double il = GetMarkInitialLoss(g_markedSymbols[s]);
         if(il > 0) targetP += il * g_unlockRatio;
      }
      if(targetP <= 0) targetP = g_minHedgeProfit * 2;
...
int OnInit()
{
   ...
   {
      ArrayResize(g_markedTickets, newSize);
      Print("[启动] 清理 ",removed," 个已失效的标记ticket");
   }

   EventSetTimer(1);
   DrawPanel();
   Print("═══════════════════════════════════════════════════");
   Print("[账户仓位多空仓位平衡风控] v5.00 启动 (仅监控, 不开仓)");
   if(g_monitorDrawdownUSD == 0)
   ...
   Print("  标记ticket数: ", ArraySize(g_markedTickets));
   Print("  标记品种数: ", ArraySize(g_markedSymbols));
...
void OnDeinit(const int reason)
{
   ...
   ObjectDelete(0, g_prefix+"btn_expand");
   EventKillTimer();
   Print("[账户仓位多空仓位平衡风控] v5 已停止 (状态已持久化)");
}

void OnTick()
{
   CleanupMarkedTickets();
   CheckMonitor();
}
