...
// ========== 融合新增参数 ==========
input int    PanelStyle = 1;                   // 面板样式: 1=信息密集, 2=视觉增强, 3=分区布局
...
// 品种排序相关变量
double g_lastProfitForSort[MAX_SYMBOLS];
bool g_sortInitialized = false;
datetime g_lastUserEditTime = 0;

int GetMonitorPanelContentHeight() {
   return TITLE_BAR_HEIGHT_MON + 4 + HEADER_HEIGHT_MON + 2 + g_symbolCount * ROW_HEIGHT_MON + 4 + FOOTER_HEIGHT_MON;
}
...
void SwapMonitorSlots(int a, int b) {
   if(a == b) return;
   string tmpStr;
   double tmpDub;
   tmpStr = g_symbols[a]; g_symbols[a] = g_symbols[b]; g_symbols[b] = tmpStr;
   tmpDub = g_thresholds[a]; g_thresholds[a] = g_thresholds[b]; g_thresholds[b] = tmpDub;
   tmpStr = g_editNames[a]; g_editNames[a] = g_editNames[b]; g_editNames[b] = tmpStr;
   tmpStr = g_labelNames[a]; g_labelNames[a] = g_labelNames[b]; g_labelNames[b] = tmpStr;
   tmpStr = g_btnCloseAllNames[a]; g_btnCloseAllNames[a] = g_btnCloseAllNames[b]; g_btnCloseAllNames[b] = tmpStr;
   tmpStr = g_btnCloseBuyNames[a]; g_btnCloseBuyNames[a] = g_btnCloseBuyNames[b]; g_btnCloseBuyNames[b] = tmpStr;
   tmpStr = g_btnCloseSellNames[a]; g_btnCloseSellNames[a] = g_btnCloseSellNames[b]; g_btnCloseSellNames[b] = tmpStr;
   tmpStr = g_profitLabelNames[a]; g_profitLabelNames[a] = g_profitLabelNames[b]; g_profitLabelNames[b] = tmpStr;
   tmpStr = g_lotsLabelNames[a]; g_lotsLabelNames[a] = g_lotsLabelNames[b]; g_lotsLabelNames[b] = tmpStr;
   tmpStr = g_countLabelNames[a]; g_countLabelNames[a] = g_countLabelNames[b]; g_countLabelNames[b] = tmpStr;
   tmpStr = g_statusLabelNames[a]; g_statusLabelNames[a] = g_statusLabelNames[b]; g_statusLabelNames[b] = tmpStr;
}

void SortSymbolsByFloatingLoss() {
   if(g_symbolCount <= 1) return;

   if(g_lastUserEditTime > 0 && TimeCurrent() - g_lastUserEditTime < 2) return;

   double profits[MAX_SYMBOLS];
   bool hasLoss = false;
   for(int i = 0; i < g_symbolCount; i++) {
      profits[i] = GetSymbolTotalProfit(g_symbols[i]);
      if(profits[i] < -1000.0) hasLoss = true;
   }

   bool needSort = false;
   for(int i = 0; i < g_symbolCount; i++) {
      double diff = MathAbs(profits[i] - g_lastProfitForSort[i]);
      if(diff > 500.0) { needSort = true; break; }
   }
   if(!g_sortInitialized) { needSort = true; g_sortInitialized = true; }
   if(!needSort) return;

   for(int i = 0; i < g_symbolCount; i++)
      g_lastProfitForSort[i] = profits[i];

   bool swapped = true;
   for(int pass = 0; pass < g_symbolCount - 1 && swapped; pass++) {
      swapped = false;
      for(int i = 0; i < g_symbolCount - 1 - pass; i++) {
         bool shouldSwap = false;
         if(hasLoss) {
            shouldSwap = (profits[i] > profits[i + 1]);
         } else {
            shouldSwap = (profits[i] < profits[i + 1]);
         }
         if(shouldSwap) {
            SwapMonitorSlots(i, i + 1);
            double tmpProfit = profits[i]; profits[i] = profits[i + 1]; profits[i + 1] = tmpProfit;
            swapped = true;
         }
      }
   }
      ...
      if(UseFloatingProfitHedge) {
         ...
         if(closeLossOK) {
            ...
            if(EnableHedgeAlert) {
               ...
               double remainLossLots = NormalizeVolumeBySymbol(maxSymbol, orderLots - hedgeLots);
               double remainProfitLots = NormalizeVolumeBySymbol(profitSymbol, profitOrderLots - profitCloseLots);
               Alert("品种:" + maxSymbol + "最大浮亏的" + lossDirStr +
                     DoubleToString(orderLots, 2) + "手，被对冲掉" +
                     DoubleToString(hedgeLots, 2) + "手(消耗" + profitSymbol + profitDirStr +
                     DoubleToString(profitCloseLots, 2) + "手)，对冲后剩余" +
                     DoubleToString(remainLossLots, 2) + "手，释放盈利" +
                     DoubleToString(profitReleaseMoney, 2) +
                     "，已完成对冲次数: " + IntegerToString(g_hedgeStatus.hedgeCount + 1) + "✅");
            }
         ...
         if(trade.PositionClosePartial((ulong)ticket, hedgeLots, 50)) {
            ...
            if(EnableHedgeAlert) {
               ...
               double remainLots = NormalizeVolumeBySymbol(maxSymbol, orderLots - hedgeLots);
               Alert("品种:" + maxSymbol + "最大浮亏的" + dirStr +
                     DoubleToString(orderLots, 2) + "手，被对冲掉" +
                     DoubleToString(hedgeLots, 2) + "手，对冲后剩余" +
                     DoubleToString(remainLots, 2) + "手，消耗当天盈利" +
                     DoubleToString(profitConsume, 2) +
                     "，消除浮亏-" + DoubleToString(profitConsume, 2) +
                     "，已完成对冲次数: " + IntegerToString(g_hedgeStatus.hedgeCount + 1) + "✅");
            }
            remainProfitConsume -= profitConsume;
            totalFloatingLoss = CalculateTotalFloatingLoss();
...
void UpdateProfitLabels() {
   ...
   for(int i = 0; i < g_symbolCount; i++) {
      ...
      string numName = PFX_MON "Num_" + IntegerToString(i);
      if(ObjectFind(0, numName) >= 0)
         ObjectSetString(0, numName, OBJPROP_TEXT, "#" + IntegerToString(i + 1));
      string symName = PFX_MON "Symbol_" + IntegerToString(i);
      if(ObjectFind(0, symName) >= 0)
         ObjectSetString(0, symName, OBJPROP_TEXT, g_symbols[i]);

      if(ObjectFind(0, g_editNames[i]) >= 0) {
         if(g_lastUserEditTime <= 0 || TimeCurrent() - g_lastUserEditTime >= 2) {
            string currentText = ObjectGetString(0, g_editNames[i], OBJPROP_TEXT);
            string expectedText = DoubleToString(g_thresholds[i], 0);
            if(StringCompare(currentText, expectedText) != 0)
               ObjectSetString(0, g_editNames[i], OBJPROP_TEXT, expectedText);
         }
      }
   }
...
void CreatePanel_InfoDense() {
   ...
   int w = 600, h = GetMonitorPanelContentHeight();
   ...
   int y = py + 35;
   int sectionH = (h - 35 - 50) / 3;

   CreateCell(pfx + "AccountBg", px + 10, y, w - 20, sectionH, GetRowBgDark(), CORNER_LEFT_UPPER, false);
   ...
   y += sectionH + 5;
   CreateCell(pfx + "HedgeBg", px + 10, y, w - 20, sectionH, GetRowBgLight(), CORNER_LEFT_UPPER, false);
   ...
   y += sectionH + 5;
   CreateCell(pfx + "MonitorBg", px + 10, y, w - 20, sectionH, GetRowBgDark(), CORNER_LEFT_UPPER, false);
   ...
   CreateLabel(pfx + "MonitorAlert", "⚠️ 超阈值品种: 0", px + 20, y + sectionH - 30, GetProfitNegColor(), 9);
   CreateLabel(pfx + "MonitorCount", "监控品种: " + IntegerToString(g_symbolCount) + " 个", px + 200, y + sectionH - 30, GetHeaderTxtColor(), 9);
   CreateLabel(pfx + "MonitorLock", g_enableLockPosition ? "🔒 锁仓: 开" : "🔓 锁仓: 关", px + 380, y + sectionH - 30, GetTitleColor(), 9);

   int btnY = py + h - 40;
   CreateCell(pfx + "BtnBg", px + 10, btnY, w - 20, 35, GetHeaderBgColor(), CORNER_LEFT_UPPER, false);
   CreateActionButton(pfx + "BtnManualHedge", "手动对冲", px + 20, btnY + 8, 80, 20);
   CreateActionButton(pfx + "BtnToggleLock", g_enableLockPosition ? "锁仓:开" : "锁仓:关", px + 110, btnY + 8, 80, 20);
   CreateActionButton(pfx + "BtnHide", "隐藏面板", px + 200, btnY + 8, 70, 20);
   CreateActionButton(pfx + "BtnScheme", "配色", px + 280, btnY + 8, 60, 20);
   CreateActionButton(pfx + "BtnCloseAll", "全平", px + 350, btnY + 8, 60, 20);
   CreateActionButton(pfx + "BtnPause", g_enableHedgeEngine ? "暂停EA" : "启动EA", px + 420, btnY + 8, 70, 20);
...
void UpdatePanel_InfoDense() {
   ...
   ObjectSetString(0, pfx + "BtnPause", OBJPROP_TEXT, g_enableHedgeEngine ? "暂停EA" : "启动EA");

   static datetime startTime = 0;
   if(startTime == 0) startTime = TimeCurrent();
   int hours = (int)((TimeCurrent() - startTime) / 3600);
   ObjectSetString(0, pfx + "Runtime", OBJPROP_TEXT, "⏱ 运行: " + IntegerToString(hours) + "h");
...
void CreatePanel_VisualEnhanced() {
   ...
   int w = 550, h = GetMonitorPanelContentHeight();
   int px = PanelX_Init, py = PanelY_Init;
   ...
   int y = py + 35;
   int sectionH = (h - 35 - 45) / 2;

   CreateCell(pfx + "MainCard", px + 10, y, w - 20, sectionH, GetRowBgDark(), CORNER_LEFT_UPPER, false);
   ...
   y += sectionH + 10;
   CreateCell(pfx + "MonitorCard", px + 10, y, w - 20, sectionH - 10, GetRowBgLight(), CORNER_LEFT_UPPER, false);
   ...
   string maxSymbol = "";
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ulong hh = PositionGetTicket(i);
      if(hh == 0) continue;
      if(PositionGetInteger(POSITION_TYPE) > POSITION_TYPE_SELL) continue;
      double loss = GetPositionNetProfitByTicket(hh);
      if(loss < maxLoss) {
         maxLoss = loss;
         ticket = (long)hh;
         maxSymbol = PositionGetString(POSITION_SYMBOL);
   ...
   int btnY = py + h - 35;
   CreateCell(pfx + "BtnCard", px + 10, btnY, w - 20, 30, GetHeaderBgColor(), CORNER_LEFT_UPPER, false);
   CreateActionButton(pfx + "BtnHedge", "手动对冲", px + 20, btnY + 5, 80, 20);
   CreateActionButton(pfx + "BtnLock", g_enableLockPosition ? "锁仓:开" : "锁仓:关", px + 110, btnY + 5, 80, 20);
   CreateActionButton(pfx + "BtnHide", g_panelVisible ? "隐藏" : "显示", px + 200, btnY + 5, 60, 20);
   CreateActionButton(pfx + "BtnScheme", "配色", px + 270, btnY + 5, 50, 20);
   CreateActionButton(pfx + "BtnCloseAll", "全平", px + 330, btnY + 5, 50, 20);
   CreateActionButton(pfx + "BtnPause", g_enableHedgeEngine ? "暂停" : "启动", px + 390, btnY + 5, 50, 20);
...
void UpdatePanel_VisualEnhanced() {
   ...
   ObjectSetString(0, pfx + "BtnPause", OBJPROP_TEXT, g_enableHedgeEngine ? "暂停" : "启动");
...
void CreatePanel_TabLayout() {
   ...
   int w = 650, h = GetMonitorPanelContentHeight();
   int px = PanelX_Init, py = PanelY_Init;
   ...
   int contentH = h - (contentY - py) - 40;
   CreateCell(pfx + "ContentBg", px + 10, contentY, w - 20, contentH, GetRowBgDark(), CORNER_LEFT_UPPER, false);
   ...
   int btnY = py + h - 35;
   CreateActionButton(pfx + "BtnManualHedge", "手动对冲", px + 20, btnY, 80, 20);
   CreateActionButton(pfx + "BtnLock", g_enableLockPosition ? "锁仓:开" : "锁仓:关", px + 110, btnY, 80, 20);
   CreateActionButton(pfx + "BtnHide", g_panelVisible ? "隐藏" : "显示", px + 200, btnY, 60, 20);
   CreateActionButton(pfx + "BtnScheme", "配色", px + 270, btnY, 50, 20);
   CreateActionButton(pfx + "BtnPause", g_enableHedgeEngine ? "暂停EA" : "启动EA", px + 330, btnY, 70, 20);
...
void CreateMonitorPanel() {
   ...
   int monH = GetMonitorPanelContentHeight();
   CreateCell(PFX_MON "Panel_Border", px - BORDER_WIDTH_MON, py - BORDER_WIDTH_MON,
              PANEL_WIDTH_MON + 2 * BORDER_WIDTH_MON, monH + 2 * BORDER_WIDTH_MON,
              GetBorderColor(), CORNER_LEFT_UPPER, true);
   CreateCell(PFX_MON "Panel_BG", px, py, PANEL_WIDTH_MON, monH,
...
int OnInit() {
   ...
   g_sortInitialized = false;
   g_lastUserEditTime = 0;
   for(int si = 0; si < MAX_SYMBOLS; si++)
      g_lastProfitForSort[si] = 0.0;
   ...
   Print("  - 面板样式: ", PanelStyle, " (1=信息密集, 2=视觉增强, 3=分区布局)");
   ...
   if(id == CHARTEVENT_OBJECT_CLICK) {
      ...
      if(StringFind(sparam, PFX_FUSION + "Info_") == 0 ||
         ...
         if(StringFind(btnName, "BtnManualHedge") > 0 || StringFind(btnName, "BtnHedge") > 0) {
            if(!g_enableHedgeEngine) {
               Alert("⚠️ 对冲引擎已禁用，请先启用对冲引擎");
               return;
            }
            if(g_hedgeStatus.todayProfit < DailyProfitThreshold) {
               Alert("⚠️ 今日盈利不足，无法启动对冲: 当前盈利$" + DoubleToString(g_hedgeStatus.todayProfit, 2) +
                     "，最低阈值$" + DoubleToString(DailyProfitThreshold, 2));
               return;
            }
            if(MessageBox("确认执行手动对冲？\n将消耗当天盈利对冲最大浮亏订单", "手动对冲确认", MB_YESNO | MB_ICONQUESTION) != IDYES)
               return;
            string lossText = "";
            long lossTicket = -1;
            GetMaxFloatingLossOrderInfo(lossText, lossTicket);
            string modeStr = UseFloatingProfitHedge ? "浮盈对冲" : "盈利消耗";
            Alert("📊 手动对冲已触发: 今日盈利$" + DoubleToString(g_hedgeStatus.todayProfit, 2) +
         ...
         if(StringFind(btnName, "BtnToggleLock") > 0 || StringFind(btnName, "BtnLock") > 0) {
            string lockMsg = g_enableLockPosition ? "确认关闭锁仓功能？\n关闭后将允许EA进行交易操作" : "确认开启锁仓功能？\n开启后EA将停止所有交易操作";
            if(MessageBox(lockMsg, "锁仓确认", MB_YESNO | MB_ICONQUESTION) != IDYES)
               return;
         ...
         if(StringFind(btnName, "BtnPause") > 0) {
            string pauseMsg = g_enableHedgeEngine ? "确认暂停对冲引擎？\n暂停后对冲引擎将停止工作" : "确认启动对冲引擎？\n启动后对冲引擎将开始工作";
            if(MessageBox(pauseMsg, "启动/暂停确认", MB_YESNO | MB_ICONQUESTION) != IDYES)
               return;
            g_enableHedgeEngine = !g_enableHedgeEngine;
            Alert(g_enableHedgeEngine ? "对冲引擎已启动" : "对冲引擎已暂停");
            UpdateActivePanel();
            return;
         }
      }
   ...
   if(id == CHARTEVENT_OBJECT_ENDEDIT) {
      for(int i = 0; i < g_symbolCount; i++) {
         if(sparam == g_editNames[i]) {
            g_lastUserEditTime = TimeCurrent();
            string text = ObjectGetString(0, g_editNames[i], OBJPROP_TEXT);
