...
struct DailyStats
{
   ...
   double   maxVolume;
   double   minVolume;
   double   deposit;   // 当日出入金（入金为正，出金为负）
};
...
int OnInit()
{
   ...
   Print("动态/静态统计网格面板(MT5)已启动，", GRID_ROWS, "行 × ", GRID_COLS, "列");
...
void CreateToggleButton()
{
   ...
   if(ObjectFind(0, TOGGLE_BTN_NAME) < 0)
   {
      ...
      {
         Print("错误：创建切换按钮失败, code:", GetLastError());
         return;
      }
...
void CreateModeButton()
{
   ...
   if(ObjectFind(0, MODE_BTN_NAME) < 0)
   {
      ...
      {
         Print("错误：创建模式切换按钮失败, code:", GetLastError());
         return;
      }
...
void ProcessOrderDataDaily()
{
   ...
   for(int i = 0; i < deals; i++)
   {
      ...
      if(g_dailyStats[idx].maxVolume == 0.0 || vol > g_dailyStats[idx].maxVolume)
         g_dailyStats[idx].maxVolume = vol;
      if(g_dailyStats[idx].minVolume == 0.0 || vol < g_dailyStats[idx].minVolume)
         g_dailyStats[idx].minVolume = vol;
   }

   // 统计出入金（余额类操作：入金/出金，按日期汇总到 deposit）
   for(int iDep = 0; iDep < deals; iDep++)
   {
      ulong dealTicket2 = HistoryDealGetTicket(iDep);
      long  dealType    = (long)HistoryDealGetInteger(dealTicket2, DEAL_TYPE);
      if(dealType != DEAL_TYPE_BALANCE && dealType != DEAL_TYPE_CREDIT)
         continue; // 只处理余额类操作

      double amount   = HistoryDealGetDouble(dealTicket2, DEAL_PROFIT);
      datetime opTime = (datetime)HistoryDealGetInteger(dealTicket2, DEAL_TIME);
      if(opTime < startTime) continue;

      datetime day2 = StringToTime(TimeToString(opTime, TIME_DATE));

      int idx2 = -1;
      for(int k2 = 0; k2 < g_dailyStatsCount; k2++)
      {
         if(g_dailyStats[k2].date == day2) { idx2 = k2; break; }
      }
      if(idx2 < 0)
      {
         idx2 = g_dailyStatsCount;
         g_dailyStatsCount++;
         ArrayResize(g_dailyStats, g_dailyStatsCount);
         g_dailyStats[idx2].date        = day2;
         g_dailyStats[idx2].profit      = 0.0;
         g_dailyStats[idx2].balance     = 0.0;
         g_dailyStats[idx2].tradeCount  = 0;
         g_dailyStats[idx2].winCount    = 0;
         g_dailyStats[idx2].volume      = 0.0;
         g_dailyStats[idx2].maxVolume   = 0.0;
         g_dailyStats[idx2].minVolume   = 0.0;
         g_dailyStats[idx2].deposit     = 0.0;
      }

      g_dailyStats[idx2].deposit += amount;
   }
   ...
   // 按“日期”记录账户最大浮亏/浮盈（当日内最大值，跨日自动使用新日期键）
   datetime todayDate  = StringToTime(TimeToString(TimeCurrent(), TIME_DATE));
   string   todayStr   = TimeToString(todayDate, TIME_DATE);           // "YYYY.MM.DD"
   string   keyMaxLoss = GV_PREFIX + "DayMaxFloatLoss_" + todayStr;
   string   keyMaxProfit = GV_PREFIX + "DayMaxFloatProfit_" + todayStr;
...
void UpdateGridDataDynamic()
{
   ...
   for(int row = 1; row <= 10; row++)
   {
      ...
      if(dataIndex < endIndex && dataIndex < g_statsCount)
      {
         ...
         if(digits <= 0) digits = (int)_Digits;
...
void UpdateGridDataDaily()
{
   ...
   for(int row = 1; row <= 10; row++)
   {
      ...
      if(dataIndex < endIndex && dataIndex < g_dailyStatsCount)
      {
         ...
         string depStr = (ds.deposit != 0.0) ? DoubleToString(ds.deposit, 2) : "-";
         UpdateCellText(row, 16, depStr);
      }
      else
      {
         UpdateCellText(row, 0, "-");
      ...
      if(dataIndex < endIndex && dataIndex < g_dailyStatsCount)
      {
         // 根据每一行对应的日期，读取当日账户最大浮亏/浮盈
         datetime rowDay     = g_dailyStats[dataIndex].date;
         string   rowDayStr  = TimeToString(rowDay, TIME_DATE);
         string   keyDayLoss   = GV_PREFIX + "DayMaxFloatLoss_" + rowDayStr;
         string   keyDayProfit = GV_PREFIX + "DayMaxFloatProfit_" + rowDayStr;

         double dayMaxLoss   = 0.0;
         double dayMaxProfit = 0.0;
         if(GlobalVariableCheck(keyDayLoss))
            dayMaxLoss = GlobalVariableGet(keyDayLoss);
         if(GlobalVariableCheck(keyDayProfit))
            dayMaxProfit = GlobalVariableGet(keyDayProfit);

         string maxLossStr   = (dayMaxLoss < 0.0)   ? FormatCompactFloat(dayMaxLoss, 2)   : "0";
         string maxProfitStr = (dayMaxProfit > 0.0) ? FormatCompactFloat(dayMaxProfit, 2) : "0";
         string lossPctStr   = "-";
         string profitPctStr = "-";
         ...
         {
            if(dayMaxLoss < 0.0)
               lossPctStr  = DoubleToString(100.0 * dayMaxLoss / bal, 1) + "%";
            if(dayMaxProfit > 0.0)
               profitPctStr = DoubleToString(100.0 * dayMaxProfit / bal, 1) + "%";
         }

         UpdateCellText(row, 6, maxLossStr,   (dayMaxLoss < 0.0 ? LossColor : clrBlack));
         UpdateCellText(row, 7, lossPctStr);
         UpdateCellText(row, 8, profitPctStr);
         UpdateCellText(row, 9, maxProfitStr, (dayMaxProfit > 0.0 ? ProfitColor : clrBlack));
      }
      else
      {
